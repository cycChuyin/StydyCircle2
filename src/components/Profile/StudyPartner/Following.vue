  ...
  <div class="container">
    <ul class="row row-cols-1 g-4 pt-32 mb-10 list-unstyled">
      <li
        ...
        v-for="item in getFollowingData"
        :key="item.Id"
      >
  ...
  created () {
    ...
    this.$apiHelper.get('api/users/profile-data').then((res) => {
      ...
      if (res.data.Status) {
        ...
        this.$apiHelper
          .get(`api/users/login/following/list/${this.routeUserId}/7/1`)
          .then((res) => {
            if (res.data.Status) {
              console.log(res.data)
              // 存新的 Token
              ...
              this.getFollowingData = oriFollowingData
              console.log(this.getFollowingData)
            }
          })
      } else {
        // 如果沒登入
        // 接 7-5 關注中的會員資料+分頁
        this.$apiHelper
          .get(`api/users/following/list/${this.routeUserId}/7/1`)
          .then((res) => {
            if (res.data.Status) {
              console.log('沒有登入')
              console.log(res.data)
