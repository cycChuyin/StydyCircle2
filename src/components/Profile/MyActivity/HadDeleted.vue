<template>
  ...
  <div class="container">
    <ul class="row row-cols-1 row-cols-md-3 g-4 list-unstyled py-32">
      <li class="col" v-for="item in getHadDeletedData" :key="item.ActivityId">
...
export default {
  ...
  created () {
    ...
    // 7-9 收藏活動資料+分頁
    this.$apiHelper
      .get(`api/users/activity/attend/cancel/${this.routeUserId}/9/1`)
      .then((res) => {
        ...
        if (res.data.Status === true) {
          ...
          this.getHadDeletedData = oriHadDeletedData
          console.log(this.getHadDeletedData)
        }
      })
  },
  methods: {
